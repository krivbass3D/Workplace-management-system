      ...
      <tr class="del_posada<?php echo $fetch['POSADA_ID']?>">
        ...
        <td>
          <center><?php if($fetch['STRUCTURE_ID']>0){?>
            <button class="btn btn-warning btn-update" data-toggle="modal" 
              data-target="#update_modal<?php echo $fetch['POSADA_ID']?>"><span
                class="glyphicon glyphicon-edit"></span></button>
            <button class="btn btn-info" data-toggle="modal" 
              data-target="#view_modal<?php echo $fetch['POSADA_ID']?>"><span
...
<!-- Просмотр заяви -->
      ...
      -->
        ...
        <div class="modal-body">
          ...
          <center><h4 class="text-warning">Вимоги до вакансії</h4></center>
          <?php 
          ...
          <div class="col-md-12">
            <div class="form-group">
              <h3><?php echo $fetch1['SECTION_ID']." "; echo $fetch1['SECTION_NAME']?></h3>
              <?php 
                    ...
                    while($fetch2 = mysqli_fetch_array($query2)){
                      ...
                      while($fetch3 = mysqli_fetch_array($query3)){
                        if ($fetch2['PARAGRAPH_ID']==$fetch3['PARAGRAPH_ID']){
                          $PARAGRAPH_NAME = $fetch3['PARAGRAPH_NAME'];
                          $STRUCTURE_NAME = $fetch3['STRUCTURE_NAME'];
                        }
                        };
                        //echo $fetch2['STRUCTURE_ID'];
                        if ($STRUCTURE_NAME != "") { ?>
                        <h4><?php echo $PARAGRAPH_NAME;?> : </h4>
                        <h5><?php echo $STRUCTURE_NAME;?></h5>
                          <?php } else {?>
                        <!--
                        <label><?php echo "Пусто";?></label>
                        -->
                        <?php }?>
                      <?php };?>
            </div>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" name="update" class="btn btn-success "><span class="glyphicon glyphicon-save"></span> Друк</button>
    ...
    <div class="modal-content">
      ...
      <form method="POST" action="update_zajava.php">
            ...
            <div class="form-group">
              ...
              <?php 
                    ...
                    while($fetch2 = mysqli_fetch_array($query2)){
                      ...
                      while($fetch3 = mysqli_fetch_array($query3)){
                        if ($fetch2['PARAGRAPH_ID']==$fetch3['PARAGRAPH_ID']){
                          $PARAGRAPH_ID = $fetch3['PARAGRAPH_ID'];
                          $STRUCTURE_NAME = $fetch3['STRUCTURE_NAME'];
                        }
                        };
                        //echo $fetch2['STRUCTURE_ID'];
                        if ($STRUCTURE_NAME != "") {
                          if ($fetch2['PARAGRAPH_ID']==31) { ?>
                            <select name="<?php echo $fetch2['PARAGRAPH_ID'];?>" class="form-control" >
                                <?php
                                if ($STRUCTURE_NAME=="Зміна співробітника") {?>
                                <option selected >Зміна співробітника</option>
                                <option>Нововведена штатна одиниця</option>
                                <?php } else { ?>
                                <option>Зміна співробітника</option>
                                <option selected >Нововведена штатна одиниця</option>
                                <?php }?>
                              </select>
                          <?php } else { ?>
                                <input type="text" id="update_<?php echo $PARAGRAPH_ID;?>" name="<?php echo $PARAGRAPH_ID;?>" value="<?php echo $STRUCTURE_NAME;?>" class="form-control">
                          <?php }?>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-dismiss="modal"><span
              class="glyphicon glyphicon-remove"></span> Закрити</button>
          <button type="submit" name="update" class="btn btn-success "><span class="glyphicon glyphicon-save"></span> Зберегти</button>
          <input type='hidden' name='POSADA_ID' data-delORadd="Додати" value="<?php echo $fetch['POSADA_ID']?>">
        </div>
      </form>
    </div>
        ...
        <div class="modal-body">
          ...
          <div class="col-md-12">
            <div class="form-group">
              ...
              <?php 
                    ...
                    while($fetch2 = mysqli_fetch_array($query2)){
                      if ($fetch2['PARAGRAPH_ID']==31) { ?>
                         <select name="<?php echo $fetch2['PARAGRAPH_ID'];?>" class="form-control" id="exampleFormControlSelect1">
                            <option>Зміна співробітника</option>
                            <option>Нововведена штатна одиниця</option>
                          </select>
                      <?php } else { ?>
          ...
          <div class="form-check form-check-inline">
            ...
            <?php 
              ...
              $query2 = mysqli_query($conn,$sql ) or die(mysqli_error());
              while($fetch2 = mysqli_fetch_array($query2)){
                if ( $fetch2['PARAGRAPH_NAME']=="Додатково") {?>
                  <input type="text" name="<?php echo $fetch2['PARAGRAPH_ID'];?>" class="form-control"
                placeholder="<?php echo $fetch2['PARAGRAPH_NAME'];?>">
                <?php } else {?>
                <input type="checkbox"  data-toggle="toggle" data-onstyle="success" data-size="mini" class = "custom-control-input" name="<?php echo $fetch2['PARAGRAPH_ID'];?>" value="1">
                <label class="form-check-label" for="inlineCheckbox1"><?php echo $fetch2['PARAGRAPH_NAME'];?></label>
                <?php }?>
